        ...
        <div class="chat-panel">
          ...
          <template v-for="(msg, index) in messages" :key="index">
            <div v-if="msg.role === 'assistant'" class="msg-incoming">
              ...
              <div class="msg-body">
                <!-- 调用链可视化 -->
                <div v-if="msg.callChain?.length" class="msg-call-chain">
                  <span v-for="(step, si) in msg.callChain" :key="si" class="call-step" :class="'call-step--' + step.status">
                    <span class="call-step-icon">{{ step.icon }}</span>
                    <span class="call-step-label">{{ step.label }}</span>
                    <span v-if="si < msg.callChain.length - 1" class="call-step-arrow">→</span>
                  </span>
                </div>
                <div
                ...
                >
                  ...
                  <!-- 错误重试按钮 -->
                  <div v-if="msg.error" class="msg-retry">
                    <button type="button" class="phro-btn retry-btn" @click="handleRetry(msg)">🔄 重新发送</button>
                  </div>
                </div>
              </div>
            </div>
...
function getToolName(toolKey) {
  const map = {
    detect_single_image: '单图检测', detect_batch_images: '批量检测',
    detect_zip_images_file: 'ZIP检测', detect_video_file: '视频检测',
    search_knowledge: '知识库检索', query_detection_stats: '统计查询',
    query_detection_history: '历史查询', get_history_records: '历史查询',
    get_statistics_overview: '数据概览', get_defect_types: '缺陷类型',
    get_task_list: '任务列表', get_task_detail: '任务详情',
    analyze_defects: '缺陷分析', calculate_pass_rate: '良品率',
    query_user_list: '用户查询', get_model_info: '模型信息',
    get_scenes: '场景列表', get_batches: '批次列表',
    get_training_tasks: '训练任务',
  }
  ...
  stopStream.value = streamChat(
    ...
    },
    {
      onMessage: (data) => {
        ...
        // 初始化调用链
        if (!last.callChain) last.callChain = []

        switch (data.type) {
          case 'thinking': {
            last.callChain.push({ icon: '🧠', label: data.content || '思考中…', status: 'active' })
            break
          }
          case 'route': {
            last.callChain[last.callChain.length - 1] = { icon: '🧠', label: '分析意图 → ' + routeLabel(data.content), status: 'done' }
            break
          }
          case 'text_chunk': {
            // 移除最后一个 pending 状态（如果有的话），改为 done
            const lastStep = last.callChain[last.callChain.length - 1]
            if (lastStep && lastStep.status === 'pending') lastStep.status = 'done'
            if (data.content) last.content = (last.content || '') + data.content
            break
          }
          case 'tool_start': {
            last.callChain.push({ icon: '⚙', label: getToolName(data.tool) || data.tool, status: 'active' })
            break
          }
          case 'tool_end': {
            // 将最后一个 active 状态的 tool_start 改为 done
            const rev = [...last.callChain].reverse()
            const toolStep = rev.find(s => s.status === 'active' && s.icon === '⚙')
            if (toolStep) {
              toolStep.icon = '✅'
            ...
            try {
              ...
              if (result && (result.total_objects !== undefined || result.task_id)) {
                last.detectionResult = result
                last.callChain.push({ icon: '📊', label: `检测到 ${result.total_objects || 0} 个目标`, status: 'done' })
              }
            } catch {}
            break
          }
          case 'error': {
            ...
            last.error = true
            last.retryContext = { message: text, imagePath, imagePreview }
            break
          }
          default: {
            // 兼容旧格式：直接 content 字段
...
function handleRetry(msg) {
  ...
  const chat = chatState()
  if (!chat) return
  // Remove current failed assistant message
  const idx = chat.messages.indexOf(msg)
  if (idx >= 0) chat.messages.splice(idx, 1)
  // Restore the user message context and re-send
  inputText.value = ctx.message || ''
  pendingFilePath.value = ctx.imagePath || ''
  pendingFilePreview.value = ctx.imagePreview || ''
  chat.isLoading = false
  stopStream.value = null
  handleSend()
}
...
.msg-call-chain {
  ...
  margin-bottom: 6px;
  padding: 6px 10px;
  background: rgba($phro-gold, 0.06);
  border: 1px solid rgba($phro-gold, 0.15);
  border-radius: 8px;
  font-size: 12px;
}
