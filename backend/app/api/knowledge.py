...
async def build_knowledge_base(current_user=Depends(get_current_user)):
    ...
    try:
        ...
        if not documents:
            return {"success": False, "message": "没有找到知识文档"}

        # 清空旧索引
        pgvector_client.clear()
        pgvector_client.init_schema()
        ...
        logger.info("Knowledge base built: %d chunks indexed", total_chunks)
        return {
            "success": True,
            "message": f"知识库构建完成",
...
async def knowledge_stats(current_user=Depends(get_current_user)):
    """知识库统计信息"""
    return {
        "total_chunks": pgvector_client.count(),
        "table_name": pgvector_client.table_name,
        "vector_dim": pgvector_client.vector_dim,
    }
