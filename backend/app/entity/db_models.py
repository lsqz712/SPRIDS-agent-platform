...
class User(Base):
    ...
    username = Column(String(50), unique=True, nullable=False, index=True, comment="⽤户名")
...
class Role(Base):
    ...
    __tablename__ = "roles"
    id = Column(Integer, primary_key=True, autoincrement=True)
    name = Column(String(50), unique=True, nullable=False, comment="⻆⾊标识，如 admin/operator/viewer")
    display_name = Column(String(100), nullable=False, comment="⻆⾊显示名，如 管理员/操作员/访客")
...
class Permission(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    code = Column(String(100), unique=True, nullable=False, comment="权限编码，如detection:task:create")
...
class DetectionScene(Base):
    ...
    display_name = Column(String(100), nullable=False, comment="场景显示名，如 遥感⽬标检测")
    ...
    class_names_cn = Column(JSON, nullable=True, comment="类别中⽂名映射，如 {\"airplane\":\"⻜机\"}")
    is_active = Column(Boolean, default=True, comment="是否启⽤")
    created_by = Column(Integer, ForeignKey("users.id"), nullable=True, comment="创建⼈")
    created_at = Column(DateTime, default=datetime.now)
...
class DetectionTask(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True, comment="操作⽤户")
    scene_id = Column(Integer, ForeignKey("detection_scenes.id"), nullable=False, index=True, comment="使⽤的检测场景")
    model_version_id = Column(Integer, ForeignKey("model_versions.id"), nullable=True,comment="使⽤的模型版本")
    ...
    risk_level = Column(String(20), nullable=True, comment="⻛险等级：low/medium/high/critical")
...
class ModelVersion(Base):
    ...
    training_task_id = Column(Integer, ForeignKey("training_tasks.id"), nullable=True,comment="来源训练任务（可为空，⽀持⼿动上传）")
...
class ChatSession(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True, comment="所属⽤户")
    session_uuid = Column(String(100), unique=True, nullable=False, index=True, comment="会话唯⼀标识")
...
class ChatMessage(Base):
    ...
    role = Column(String(20), nullable=False, comment="消息⻆⾊：user/assistant/tool/system")
    content = Column(Text, nullable=False, comment="消息内容")
    # 智能体路由信息
    agent_used = Column(String(50), nullable=True, comment="处理的 Agent：supervisor/detection/analysis/qa")
    tool_calls = Column(JSON, nullable=True, comment="⼯具调⽤记录，如 [{\"tool\":\"detect_objects\",\"args\":{...}}]")
